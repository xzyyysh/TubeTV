...
public class TeleUniversoExtractor {
    private static final String TAG = "TeleUniversoExtractor";
    private static final String PAGE_URL = "https://teleuniversotv.com/en-vivo/";
    ...
    public static void extractStreamUrl(ExtractionCallback callback) {
        executor.execute(() -> {
            for (int attempt = 1; attempt <= MAX_RETRIES; attempt++) {
                try {
                    String pageHtml = fetchContent(PAGE_URL, "https://teleuniversotv.com/");
                    if (pageHtml == null) throw new Exception("Failed to fetch main page");

                    String iframeUrl = extractPattern(pageHtml, "iframe[^>]+src=[\"']([^\"']*tmcreativos\\.com[^\"']+)[\"']");
                    ...
                    String streamUrl = extractPattern(playerHtml, "https?://[^\"'\\s]*\\.m3u8[^\"'\\s]*");
                    if (streamUrl == null) throw new Exception("No m3u8 URL found in player");
