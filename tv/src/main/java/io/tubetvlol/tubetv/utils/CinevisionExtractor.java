...
public class CinevisionExtractor {
    private static final String TAG = "CinevisionExtractor";
    private static final String PAGE_URL = "https://canal19.do/";
    ...
    public static void extractStreamUrl(ExtractionCallback callback) {
        executor.execute(() -> {
            for (int attempt = 1; attempt <= MAX_RETRIES; attempt++) {
                try {
                    String pageHtml = fetchContent(PAGE_URL, "https://canal19.do/");
                    if (pageHtml == null) throw new Exception("Failed to fetch main page");

                    String iframeUrl = extractPattern(pageHtml, "iframe[^>]+src=[\"']([^\"']*mojocp\\.com[^\"']+)[\"']");
                    ...
                    String streamUrl = extractPattern(playerHtml, "\"src\"\\s*:\\s*\"(https?://[^\"]+\\.m3u8[^\"]*)\"");
