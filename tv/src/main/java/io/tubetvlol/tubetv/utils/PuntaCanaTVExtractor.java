...
public class PuntaCanaTVExtractor {
    private static final String TAG = "PuntaCanaTVExtractor";
    private static final String PAGE_URL = "https://puntacanatvrd.com/web/envivo/";
    ...
    public static void extractStreamUrl(ExtractionCallback callback) {
        executor.execute(() -> {
            for (int attempt = 1; attempt <= MAX_RETRIES; attempt++) {
                try {
                    String pageHtml = fetchContent(PAGE_URL, "https://puntacanatvrd.com/");
                    if (pageHtml == null) throw new Exception("Failed to fetch main page");

                    String iframeUrl = extractPattern(pageHtml, "iframe[^>]+src=[\"']([^\"']*hostlagarto[^\"']+)[\"']");
